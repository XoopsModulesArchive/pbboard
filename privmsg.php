 ...
 *
 * $Id: privmsg.php,v 1.96.2.32 2005/06/12 12:49:01 psotfx Exp $
 ***************************************************************************/
...
if (isset($_POST['folder']) || isset($_GET['folder'])) {
    ...
    if ('inbox' != $folder && 'outbox' != $folder && 'sentbox' != $folder && 'savebox' != $folder) {
        $folder = 'inbox';
    }
} else {
    $folder = 'inbox';
...
//
$inbox_img = ('inbox' != $folder || '' != $mode) ? '<a href="' . append_sid("privmsg.$phpEx?folder=inbox") . '"><img src="' . $images['pm_inbox'] . '" border="0" alt="' . $lang['Inbox'] . '"></a>' : '<img src="' . $images['pm_inbox'] . '" border="0" alt="' . $lang['Inbox'] . '">';
$inbox_url = ('inbox' != $folder || '' != $mode) ? '<a href="' . append_sid("privmsg.$phpEx?folder=inbox") . '">' . $lang['Inbox'] . '</a>' : $lang['Inbox'];
$outbox_img = ('outbox' != $folder || '' != $mode) ? '<a href="' . append_sid("privmsg.$phpEx?folder=outbox") . '"><img src="' . $images['pm_outbox'] . '" border="0" alt="' . $lang['Outbox'] . '"></a>' : '<img src="' . $images['pm_outbox'] . '" border="0" alt="' . $lang['Outbox'] . '">';
$outbox_url = ('outbox' != $folder || '' != $mode) ? '<a href="' . append_sid("privmsg.$phpEx?folder=outbox") . '">' . $lang['Outbox'] . '</a>' : $lang['Outbox'];
$sentbox_img = ('sentbox' != $folder || '' != $mode) ? '<a href="' . append_sid("privmsg.$phpEx?folder=sentbox") . '"><img src="' . $images['pm_sentbox'] . '" border="0" alt="' . $lang['Sentbox'] . '"></a>' : '<img src="' . $images['pm_sentbox'] . '" border="0" alt="' . $lang['Sentbox'] . '">';
$sentbox_url = ('sentbox' != $folder || '' != $mode) ? '<a href="' . append_sid("privmsg.$phpEx?folder=sentbox") . '">' . $lang['Sentbox'] . '</a>' : $lang['Sentbox'];
$savebox_img = ('savebox' != $folder || '' != $mode) ? '<a href="' . append_sid("privmsg.$phpEx?folder=savebox") . '"><img src="' . $images['pm_savebox'] . '" border="0" alt="' . $lang['Savebox'] . '"></a>' : '<img src="' . $images['pm_savebox'] . '" border="0" alt="' . $lang['Savebox'] . '">';
$savebox_url = ('savebox' != $folder || '' != $mode) ? '<a href="' . append_sid("privmsg.$phpEx?folder=savebox") . '">' . $lang['Savebox'] . '</a>' : $lang['Savebox'];
...
if ('newpm' == $mode) {
    ...
    include $phpbb_root_path . 'includes/nowrap_header.' . $phpEx;
    ...
    if ($userdata['session_logged_in']) {
        if ($userdata['user_new_privmsg']) {
            $l_new_message = (1 == $userdata['user_new_privmsg']) ? $lang['You_new_pm'] : $lang['You_new_pms'];
        } else {
...
'L_MESSAGE' => $l_new_message,
    ...
    switch ($folder) {
        case 'inbox':
            ...
            $pm_sql_user = 'AND pm.privmsgs_to_userid = ' . $userdata['uid'] . '
...
OR pm.privmsgs_type = ' . PRIVMSGS_UNREAD_MAIL . ' )';
        ...
        case 'outbox':
            ...
            $pm_sql_user = 'AND pm.privmsgs_from_userid = ' . $userdata['uid'] . '
...
OR ( pm.privmsgs_from_userid = ' . $userdata['uid'] . '
AND pm.privmsgs_type = ' . PRIVMSGS_SAVED_OUT_MAIL . ' )
    ...
    $sql = 'SELECT u.uname AS username_1, u.uid AS user_id_1, u2.uname AS username_2, u2.uid AS user_id_2, u.user_sig_bbcode_uid, u.posts, u.user_from, u.url, u.email, u.user_icq, u.user_aim, u.user_yim, u.user_regdate, u.user_msnm, u.user_viewemail, u.rank, u.user_sig, u.user_avatar, pm.*, pmt.privmsgs_bbcode_uid, pmt.privmsgs_text
...
AND u2.uid = pm.privmsgs_to_userid";
    ...
    if ((PRIVMSGS_NEW_MAIL == $privmsg['privmsgs_type'] || PRIVMSGS_UNREAD_MAIL == $privmsg['privmsgs_type']) && 'inbox' == $folder) {
...
VALUES ('
               . PRIVMSGS_SENT_MAIL
               . ", '"
               . str_replace("\'", "''", addslashes($privmsg['privmsgs_subject']))
               . "', "
...
'edit' => '<a href="' . $post_urls['edit'] . '">' . $lang['Edit_pm'] . '</a>',
    ...
    if ('inbox' == $folder) {
        ...
        $l_box_name = $lang['Outbox'];
    } elseif ('savebox' == $folder) {
        if (PRIVMSGS_SAVED_IN_MAIL == $privmsg['privmsgs_type']) {
            $post_img = $post_icons['post_img'];
...
'S_HIDDEN_FIELDS' => $s_hidden_fields,
    ...
    if (!empty($privmsg['user_viewemail']) || ADMIN == $userdata['user_level']) {
    ...
    $www_img = ($privmsg['url']) ? '<a href="' . $privmsg['url'] . '" target="_userwww"><img src="' . $images['icon_www'] . '" alt="' . $lang['Visit_website'] . '" title="' . $lang['Visit_website'] . '" border="0"></a>' : '';

    $www = ($privmsg['url']) ? '<a href="' . $privmsg['url'] . '" target="_userwww">' . $lang['Visit_website'] . '</a>' : '';
    ...
    if ($board_config['allow_sig']) {
        $user_sig = ($privmsg['privmsgs_from_userid'] == $userdata['uid']) ? $userdata['user_sig'] : $privmsg['user_sig'];
    } else {
        $user_sig = '';
    }

    $user_sig_bbcode_uid = ($privmsg['privmsgs_from_userid'] == $userdata['uid']) ? $userdata['user_sig_bbcode_uid'] : $privmsg['user_sig_bbcode_uid'];
...
'MESSAGE_TEXT' => (1 == count($mark_list)) ? $lang['Confirm_delete_pm'] : $lang['Confirm_delete_pms'],
'L_YES' => $lang['Yes'],
'L_NO' => $lang['No'],
...
'S_HIDDEN_FIELDS' => $s_hidden_fields,
        ...
        if ($delete_all) {
            switch ($folder) {
                case 'inbox':
                    $delete_type = 'privmsgs_to_userid = ' . $userdata['uid'] . ' AND (
privmsgs_type = ' . PRIVMSGS_READ_MAIL . ' OR privmsgs_type = ' . PRIVMSGS_NEW_MAIL . ' OR privmsgs_type = ' . PRIVMSGS_UNREAD_MAIL . ' )';
                    break;
                case 'outbox':
                    $delete_type = 'privmsgs_from_userid = ' . $userdata['uid'] . ' AND ( privmsgs_type = ' . PRIVMSGS_NEW_MAIL . ' OR privmsgs_type = ' . PRIVMSGS_UNREAD_MAIL . ' )';
                    break;
                case 'sentbox':
                    $delete_type = 'privmsgs_from_userid = ' . $userdata['uid'] . ' AND privmsgs_type = ' . PRIVMSGS_SENT_MAIL;
                    break;
                case 'savebox':
                    $delete_type = '( ( privmsgs_from_userid = ' . $userdata['uid'] . '
AND privmsgs_type = ' . PRIVMSGS_SAVED_OUT_MAIL . ' )
OR ( privmsgs_to_userid = ' . $userdata['uid'] . '
...
WHERE $delete_type";
        ...
        if (count($mark_list)) {
            ...
            if ('inbox' == $folder || 'outbox' == $folder) {
                switch ($folder) {
                    case 'inbox':
                        $sql = 'privmsgs_to_userid = ' . $userdata['uid'];
                        break;
                    case 'outbox':
                        $sql = 'privmsgs_from_userid = ' . $userdata['uid'];
                        break;
                }
...
AND ";
            ...
            switch ($folder) {
                case 'inbox':
                    $delete_sql .= 'privmsgs_to_userid = ' . $userdata['uid'] . ' AND (
privmsgs_type = ' . PRIVMSGS_READ_MAIL . ' OR privmsgs_type = ' . PRIVMSGS_NEW_MAIL . ' OR privmsgs_type = ' . PRIVMSGS_UNREAD_MAIL . ' )';
                    break;
                case 'outbox':
                    $delete_sql .= 'privmsgs_from_userid = ' . $userdata['uid'] . ' AND (
privmsgs_type = ' . PRIVMSGS_NEW_MAIL . ' OR privmsgs_type = ' . PRIVMSGS_UNREAD_MAIL . ' )';
                    break;
                case 'sentbox':
                    $delete_sql .= 'privmsgs_from_userid = ' . $userdata['uid'] . ' AND privmsgs_type = ' . PRIVMSGS_SENT_MAIL;
                    break;
                case 'savebox':
                    $delete_sql .= '( ( privmsgs_from_userid = ' . $userdata['uid'] . '
AND privmsgs_type = ' . PRIVMSGS_SAVED_OUT_MAIL . ' )
OR ( privmsgs_to_userid = ' . $userdata['uid'] . '
AND privmsgs_type = ' . PRIVMSGS_SAVED_IN_MAIL . ' ) )';
                    break;
...
} elseif ($save && $mark_list && 'savebox' != $folder && 'outbox' != $folder) {
    if (!$userdata['session_logged_in']) {
        redirect(append_sid("login.$phpEx?redirect=privmsg.$phpEx&folder=inbox", true));
    }
...
FROM ' . PRIVMSGS_TABLE . '
WHERE ( ( privmsgs_to_userid = ' . $userdata['uid'] . '
AND privmsgs_type = ' . PRIVMSGS_SAVED_IN_MAIL . ' )
OR ( privmsgs_from_userid = ' . $userdata['uid'] . '
AND privmsgs_type = ' . PRIVMSGS_SAVED_OUT_MAIL . ') )';
...
WHERE privmsgs_from_userid = ' . $userdata['uid'] . '
...
AND privmsgs_type = ' . PRIVMSGS_SENT_MAIL;
    ...
    $attach_sig = ($submit || $refresh) ? ((!empty($_POST['attach_sig'])) ? true : 0) : $userdata['attachsig'];
...
WHERE privmsgs_from_userid = ' . $userdata['uid'];
    ...
    if ($submit) {
        if (!empty($_POST['username'])) {
            ...
            $sql = 'SELECT uid, user_notify_pm, email, user_lang, level
FROM ' . USERS_TABLE . "
WHERE uname = '" . str_replace("\'", "''", $to_username) . "'
AND uid <> " . ANONYMOUS;
...
OR privmsgs_type = ' . PRIVMSGS_UNREAD_MAIL . ' )
AND privmsgs_to_userid = ' . $to_userdata['uid'];
...
VALUES (' . PRIVMSGS_NEW_MAIL . ", '" . str_replace("\'", "''", $privmsg_subject) . "', " . $userdata['uid'] . ', ' . $to_userdata['uid'] . ", $msg_time, '$user_ip', $html_on, $bbcode_on, $smilies_on, $attach_sig)";
        } else {
            $sql_info = 'UPDATE ' . PRIVMSGS_TABLE . '
SET privmsgs_type = ' . PRIVMSGS_NEW_MAIL . ", privmsgs_subject = '" . str_replace("\'", "''", $privmsg_subject) . "', privmsgs_from_userid = " . $userdata['uid'] . ', privmsgs_to_userid = ' . $to_userdata['uid'] . ", privmsgs_date = $msg_time, privmsgs_ip = '$user_ip', privmsgs_enable_html = $html_on, privmsgs_enable_bbcode = $bbcode_on, privmsgs_enable_smilies = $smilies_on, privmsgs_attach_sig = $attach_sig
...
WHERE uid = ' . $to_userdata['uid'];

            if (!$status = $db->sql_query($sql)) {
                message_die(GENERAL_ERROR, 'Could not update private message new/read status for user', '', __LINE__, __FILE__, $sql);
            }

            if ($to_userdata['user_notify_pm'] && !empty($to_userdata['email']) && $to_userdata['level']) {
...
AND u.uid = pm.privmsgs_from_userid";
            ...
            if ($postrow = $db->sql_fetchrow($result)) {
                if ($userdata['uid'] != $postrow['user_id']) {
        ...
        if (!$privmsg_id && ('reply' == $mode || 'edit' == $mode || 'quote' == $mode)) {
            message_die(GENERAL_ERROR, $lang['No_post_id']);
        }
...
AND uid <> " . ANONYMOUS;
            ...
            if ($row = $db->sql_fetchrow($result)) {
                $to_username = $row['uname'];
            }
        }

        if ('edit' == $mode) {
            $sql = 'SELECT pm.*, pmt.privmsgs_bbcode_uid, pmt.privmsgs_text, u.uname, u.uid, u.user_sig
FROM ' . PRIVMSGS_TABLE . ' pm, ' . PRIVMSGS_TEXT_TABLE . ' pmt, ' . USERS_TABLE . " u
WHERE pm.privmsgs_id = $privmsg_id
AND pmt.privmsgs_text_id = pm.privmsgs_id
AND pm.privmsgs_from_userid = " . $userdata['uid'] . '
AND ( pm.privmsgs_type = ' . PRIVMSGS_NEW_MAIL . '
OR pm.privmsgs_type = ' . PRIVMSGS_UNREAD_MAIL . ' )
AND u.uid = pm.privmsgs_to_userid';
            ...
            $privmsg_bbcode_enabled = (1 == $privmsg['privmsgs_enable_bbcode']);
            ...
            $user_sig = ($board_config['allow_sig']) ? ((PRIVMSGS_NEW_MAIL == $privmsg['privmsgs_type']) ? $user_sig : $privmsg['user_sig']) : '';

            $to_username = $privmsg['uname'];

            $to_userid = $privmsg['uid'];
        } elseif ('reply' == $mode || 'quote' == $mode) {
            $sql = 'SELECT pm.privmsgs_subject, pm.privmsgs_date, pmt.privmsgs_bbcode_uid, pmt.privmsgs_text, u.uname, u.uid
FROM ' . PRIVMSGS_TABLE . ' pm, ' . PRIVMSGS_TEXT_TABLE . ' pmt, ' . USERS_TABLE . " u
WHERE pm.privmsgs_id = $privmsg_id
AND pmt.privmsgs_text_id = pm.privmsgs_id
AND pm.privmsgs_to_userid = " . $userdata['uid'] . '
AND u.uid = pm.privmsgs_from_userid';
    ...
    if (!$userdata['user_allow_pm'] && 'edit' != $mode) {
        $message = $lang['Cannot_send_privmsg'];
    ...
    if ($preview && !$error) {
        ...
        if ($attach_sig && '' != $user_sig) {
            $preview_message .= '<br><br>_________________<br>' . $user_sig;
        }
...
'S_HTML_CHECKED' => (!$html_on) ? ' checked' : '',
'S_BBCODE_CHECKED' => (!$bbcode_on) ? ' checked' : '',
'S_SMILIES_CHECKED' => (!$smilies_on) ? ' checked' : '',
'S_SIGNATURE_CHECKED' => ($attach_sig) ? ' checked' : '',
...
WHERE privmsgs_type = ' . PRIVMSGS_NEW_MAIL . '
AND privmsgs_to_userid = ' . $userdata['uid'];
if (!$db->sql_query($sql)) {
...
FROM ' . PRIVMSGS_TABLE . ' ';
$sql = 'SELECT pm.privmsgs_type, pm.privmsgs_id, pm.privmsgs_date, pm.privmsgs_subject, u.uid, u.uname
FROM ' . PRIVMSGS_TABLE . ' pm, ' . USERS_TABLE . ' u ';
switch ($folder) {
    case 'inbox':
        $sql_tot .= 'WHERE privmsgs_to_userid = ' . $userdata['uid'] . '
AND ( privmsgs_type = ' . PRIVMSGS_NEW_MAIL . '
OR privmsgs_type = ' . PRIVMSGS_READ_MAIL . '
OR privmsgs_type = ' . PRIVMSGS_UNREAD_MAIL . ' )';
        $sql .= 'WHERE pm.privmsgs_to_userid = ' . $userdata['uid'] . '
AND u.uid = pm.privmsgs_from_userid
AND ( pm.privmsgs_type = ' . PRIVMSGS_NEW_MAIL . '
...
OR privmsgs_type = ' . PRIVMSGS_UNREAD_MAIL . ' )';
        break;
    case 'outbox':
        $sql_tot .= 'WHERE privmsgs_from_userid = ' . $userdata['uid'] . '
AND ( privmsgs_type = ' . PRIVMSGS_NEW_MAIL . '
OR privmsgs_type = ' . PRIVMSGS_UNREAD_MAIL . ' )';
        $sql .= 'WHERE pm.privmsgs_from_userid = ' . $userdata['uid'] . '
AND u.uid = pm.privmsgs_to_userid
AND ( pm.privmsgs_type = ' . PRIVMSGS_NEW_MAIL . '
OR privmsgs_type = ' . PRIVMSGS_UNREAD_MAIL . ' )';
        break;
...
'L_FROM_OR_TO' => ('inbox' == $folder || 'savebox' == $folder) ? $lang['From'] : $lang['To'],
...
if ($row = $db->sql_fetchrow($result)) {
    do {
        ...
        $icon_flag = (PRIVMSGS_NEW_MAIL == $flag || PRIVMSGS_UNREAD_MAIL == $flag) ? $images['pm_unreadmsg'] : $images['pm_readmsg'];

        $icon_flag_alt = (PRIVMSGS_NEW_MAIL == $flag || PRIVMSGS_UNREAD_MAIL == $flag) ? $lang['Unread_message'] : $lang['Read_message'];

        $msg_userid = $row['uid'];

        $msg_username = $row['uname'];
        ...
        if (PRIVMSGS_NEW_MAIL == $flag && 'inbox' == $folder) {
