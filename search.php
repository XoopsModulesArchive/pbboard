 ...
 *
 * $Id: search.php,v 1.72.2.9 2005/05/29 14:23:48 acydburn Exp $
 ***************************************************************************/
...
if (isset($_POST['search_author']) || isset($_GET['search_author'])) {
    $search_author = $_POST['search_author'] ?? $_GET['search_author'];
} else {
    $search_author = '';
}
$search_id = $_GET['search_id'] ?? '';
$show_results = $_POST['show_results'] ?? 'posts';
if (isset($_POST['search_terms'])) {
    $search_terms = ('all' == $_POST['search_terms']) ? 1 : 0;
} else {
    $search_terms = 0;
}
if (isset($_POST['search_fields'])) {
    $search_fields = ('all' == $_POST['search_fields']) ? 1 : 0;
} else {
    $search_fields = 0;
}
...
if (isset($_POST['sort_dir'])) {
    $sort_dir = ('DESC' == $_POST['sort_dir']) ? 'DESC' : 'ASC';
} else {
    $sort_dir = 'DESC';
}
...
$sort_by_types = [$lang['Sort_Time'], $lang['Sort_Post_Subject'], $lang['Sort_Topic_Title'], $lang['Sort_Author'], $lang['Sort_Forum']];
//
...
if ('searchuser' == $mode) {
    ...
    $store_vars = ['search_results', 'total_match_count', 'split_search', 'sort_by', 'sort_dir', 'show_results', 'return_chars'];
    ...
    if ('newposts' == $search_id || 'egosearch' == $search_id || 'unanswered' == $search_id || '' != $search_keywords || '' != $search_author) {
        if ('newposts' == $search_id || 'egosearch' == $search_id || ('' != $search_author && '' == $search_keywords)) {
            if ('newposts' == $search_id) {
...
WHERE post_time >= ' . $userdata['user_lastvisit'];
                ...
                $sort_dir = 'DESC';
            } elseif ('egosearch' == $search_id) {
                if ($userdata['session_logged_in']) {
                    $sql = 'SELECT post_id
FROM ' . POSTS_TABLE . '
WHERE poster_id = ' . $userdata['uid'];
                } else {
                ...
                $sql = 'SELECT uid
FROM ' . USERS_TABLE . "
WHERE uname LIKE '" . str_replace("\'", "''", $search_author) . "'";
...
WHERE poster_id IN ($matching_userids)";
            ...
            $split_search = (!mb_strstr($multibyte_charset, $lang['ENCODING'])) ? split_words(clean_words('search', stripslashes($search_keywords), $stopword_array, $synonym_array), 'search') : preg_split(' ', $search_keywords);

            $search_msg_only = (!$search_fields) ? 'AND m.title_match = 0' : ((mb_strstr($multibyte_charset, $lang['ENCODING'])) ? '' : '');
            ...
            for ($i = 0, $iMax = count($split_search); $i < $iMax; $i++) {
                switch ($split_search[$i]) {
                    ...
                    default:
                        ...
                        if (!mb_strstr($multibyte_charset, $lang['ENCODING'])) {
...
$search_msg_only";
                        ...
                        $row = [];
                        while (false !== ($temp_row = $db->sql_fetchrow($result))) {
        ...
        if ($total_match_count) {
            if ('topics' == $show_results) {
                ...
                if (count($search_ids) > $limiter) {
                    for ($i = 0, $iMax = count($search_ids); $i < $iMax; $i++) {
                        if ($count == $limiter) {
                            $chunk++;
                ...
                for ($i = 0, $iMax = count($search_id_chunks); $i < $iMax; $i++) {
                    $where_sql = '';
...
GROUP BY topic_id";
                        ...
                        if ('' != $search_author) {
                            ...
                            $where_sql .= " AND u.uid = p.poster_id AND u.uname LIKE '$search_author' ";
                        }
...
WHERE session_id NOT IN (' . implode(', ', $delete_search_ids) . ')';
        ...
        $per_page = ('posts' == $show_results) ? $board_config['posts_per_page'] : $board_config['topics_per_page'];
        ...
        */
        ...
        for ($i = 0, $iMax = count($store_vars); $i < $iMax; $i++) {
...
WHERE session_id = '" . $userdata['sess_id'] . "'";

        if (!($result = $db->sql_query($sql)) || !$db->sql_affectedrows()) {
            $sql = 'INSERT INTO ' . SEARCH_TABLE . " (search_id, session_id, search_array)
VALUES($search_id, '" . $userdata['sess_id'] . "', '" . str_replace("\'", "''", $result_array) . "')";
...
AND session_id = '" . $userdata['sess_id'] . "'";
    ...
    if ('' != $search_results) {
        if ('posts' == $show_results) {
            $sql = 'SELECT pt.post_text, pt.bbcode_uid, pt.post_subject, p.*, f.forum_id, f.forum_name, t.*, u.uname, u.uid, u.user_sig, u.user_sig_bbcode_uid
...
AND p.poster_id = u.uid";
            ...
            $sql = 'SELECT t.*, f.forum_id, f.forum_name, u.uname, u.uid, u2.uname as user2, u2.uid as id2, p.post_username, p2.post_username AS post_username2, p2.post_time
...
AND u2.uid = p2.poster_id";
        ...
        switch ($sort_by) {
            case 1:
                $sql .= ('posts' == $show_results) ? 'pt.post_subject' : 't.topic_title';
                break;
            case 2:
                $sql .= 't.topic_title';
                break;
            ...
            case 4:
                ...
                break;
            default:
                $sql .= ('posts' == $show_results) ? 'p.post_time' : 'p2.post_time';
                break;
        }
        ...
        $l_search_matches = (1 == $total_match_count) ? sprintf($lang['Found_search_match'], $total_match_count) : sprintf($lang['Found_search_matches'], $total_match_count);
...
'L_TOPIC' => $lang['Topic'],
        ...
        for ($j = 0, $jMax = count($split_search); $j < $jMax; $j++) {
            $split_word = $split_search[$j];

            if ('and' != $split_word && 'or' != $split_word && 'not' != $split_word) {
                $highlight_match[] = '#\b(' . str_replace('*', "([\w]+)?", $split_word) . ')\b#is';

                $highlight_active .= ' ' . $split_word;

                for ($k = 0, $kMax = count($synonym_array); $k < $kMax; $k++) {
                    [$replace_synonym, $match_synonym] = preg_split(' ', trim(mb_strtolower($synonym_array[$k])));
        ...
        for ($i = 0, $iMax = count($searchset); $i < $iMax; $i++) {
            ...
            if ('posts' == $show_results) {
                            ...
                            if (preg_match('/<.*>/', $message)) {
                                ...
                                while ($start_html = mb_strpos($message, '<', $start_html)) {
                                    ...
                                    if ($end_html = mb_strpos($message, '>', $start_html)) {
                                        ...
                                        if (1 != mb_strrpos(' ' . $hold_string, '<')) {
                                            $end_html = $start_html + 1;

                                            $end_counter = 1;

                                            while ($end_counter && $end_html < mb_strlen($message)) {
                                                if ('>' == mb_substr($message, $end_html, 1)) {
                                                    $end_counter--;
                                                } elseif ('<' == mb_substr($message, $end_html, 1)) {
                                                    $end_counter++;
                                                }
                ...
                $poster = (ANONYMOUS != $searchset[$i]['uid']) ? '<a href="' . append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . '=' . $searchset[$i]['uid']) . '">' : '';

                $poster .= (ANONYMOUS != $searchset[$i]['uid']) ? $searchset[$i]['uname'] : (('' != $searchset[$i]['post_username']) ? $searchset[$i]['post_username'] : $lang['Guest']);

                $poster .= (ANONYMOUS != $searchset[$i]['uid']) ? '</a>' : '';
...
'U_FORUM' => $forum_url,
                ...
                if (TOPIC_MOVED == $searchset[$i]['topic_status']) {
                    ...
                    if (TOPIC_LOCKED == $searchset[$i]['topic_status']) {
                        $folder = $images['folder_locked'];

                        $folder_new = $images['folder_locked_new'];
                    } elseif (POST_ANNOUNCE == $searchset[$i]['topic_type']) {
                        $folder = $images['folder_announce'];

                        $folder_new = $images['folder_announce_new'];
                    } elseif (POST_STICKY == $searchset[$i]['topic_type']) {
                        ...
                        if ($searchset[$i]['post_time'] > $userdata['user_lastvisit']) {
                            if (!empty($tracking_topics) || !empty($tracking_forums) || isset($HTTP_COOKIE_VARS[$board_config['cookie_name'] . '_f_all'])) {
                                ...
                                if ($unread_topics) {
                                    ...
                                    $newest_post_img = '<a href="'
                                                       . append_sid("viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;view=newest")
                                                       . '"><img src="'
                                                       . $images['icon_newest_reply']
                                                       . '" alt="'
                                    ...
                                    $folder_alt = (TOPIC_LOCKED == $searchset[$i]['topic_status']) ? $lang['Topic_locked'] : $lang['No_new_posts'];
                ...
                $topic_author = (ANONYMOUS != $searchset[$i]['uid']) ? '<a href="' . append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . '=' . $searchset[$i]['uid']) . '">' : '';

                $topic_author .= (ANONYMOUS != $searchset[$i]['uid']) ? $searchset[$i]['uname'] : (('' != $searchset[$i]['post_username']) ? $searchset[$i]['post_username'] : $lang['Guest']);

                $topic_author .= (ANONYMOUS != $searchset[$i]['uid']) ? '</a>' : '';
                ...
                $last_post_author = (ANONYMOUS == $searchset[$i]['id2']) ? (('' != $searchset[$i]['post_username2']) ? $searchset[$i]['post_username2'] . ' ' : $lang['Guest'] . ' ') : '<a href="'
                                                                                                                                                                                        . append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . '=' . $searchset[$i]['id2'])
                                                                                                                                                                                        . '">'
                                                                                                                                                                                        . $searchset[$i]['user2']
                                                                                                                                                                                        . '</a>';

                $last_post_url = '<a href="'
                                    . append_sid("viewtopic.$phpEx?" . POST_POST_URL . '=' . $searchset[$i]['topic_last_post_id'])
                                    . '#'
                                    . $searchset[$i]['topic_last_post_id']
                                    . '"><img src="'
                                    . $images['icon_latest_reply']
                                    . '" alt="'
...
for ($i = 100; $i < 1100; $i += 100) {
    $selected = (200 == $i) ? ' selected="selected"' : '';
...
//
$s_sort_by = '';
for ($i = 0, $iMax = count($sort_by_types); $i < $iMax; $i++) {
