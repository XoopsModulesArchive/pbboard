 ...
 *
 * $Id: pm_attachments.php,v 1.21 2005/01/22 11:33:35 acydburn Exp $
 ***************************************************************************/
...
class attach_pm extends attach_parent
{
    ...
    public function preview_attachments()
    {
        ...
        if ((!(int)$attach_config['allow_pm_attach']) && (ADMIN != $userdata['user_level'])) {
            return (false);
        }
    ...
    public function insert_attachment_pm($a_privmsgs_id)
    {
        ...
        if ((!empty($a_privmsgs_id)) && ('post' == $mode || 'reply' == $mode || 'edit' == $mode) && (((int)$attach_config['allow_pm_attach']) || (ADMIN == $userdata['user_level']))) {
...
WHERE privmsgs_id = ' . $a_privmsgs_id;
    ...
    public function duplicate_attachment_pm($switch_attachment, $original_privmsg_id, $new_privmsg_id)
    {
        ...
        if ((PRIVMSGS_NEW_MAIL == $privmsg['privmsgs_type'] || PRIVMSGS_UNREAD_MAIL == $privmsg['privmsgs_type']) && 'inbox' == $folder && 1 == (int)$switch_attachment) {
            $sql = 'SELECT *
...
WHERE privmsgs_id = ' . $new_privmsg_id;
    ...
    public function display_attach_box_limits()
    {
        ...
        if (0 == (int)$attach_config['pm_filesize_limit']) {
            $pm_filesize_limit = (int)$attach_config['attachment_quota'];
        } else {
            $pm_filesize_limit = (int)$attach_config['pm_filesize_limit'];
        }
...
'ATTACH_BOX_SIZE_STATUS' => $l_box_size_status,
    ...
    public function privmsgs_attachment_mod($mode)
    {
        ...
        $mark_list = (!empty($_POST['mark'])) ? $_POST['mark'] : [];
