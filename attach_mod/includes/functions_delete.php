 ...
 *
 * $Id: functions_delete.php,v 1.10 2005/02/23 13:15:29 acydburn Exp $
 ***************************************************************************/
...
function delete_attachment($post_id_array = -1, $attach_id_array = -1, $page = -1, $user_id = -1)
{
    ...
    if ((-1 == $post_id_array) && (-1 == $attach_id_array) && (-1 == $page)) {
        return;
    }

    if ((-1 == $post_id_array) && (-1 != $attach_id_array)) {
        $post_id_array = [];

        if (!is_array($attach_id_array)) {
            if (mb_strstr($attach_id_array, ', ')) {
                $attach_id_array = explode(', ', $attach_id_array);
            } elseif (mb_strstr($attach_id_array, ',')) {
                $attach_id_array = explode(',', $attach_id_array);
            } else {
...
GROUP BY ' . $p_id;
        ...
        for ($i = 0; $i < $num_post_list; $i++) {
            $post_id_array[] = (int)$post_list[$i][$p_id];
        }
    }
    ...
    if (-1 == $attach_id_array) {
        ...
        if (0 == $num_attach_list) {
            return;
        }

        for ($i = 0; $i < $num_attach_list; $i++) {
            $attach_id_array[] = (int)$attach_list[$i]['attach_id'];
        }
    }
    ...
    if (PAGE_PRIVMSGS == $page) {
        ...
        if (-1 != $user_id) {
            ...
            for ($i = 0, $iMax = count($post_id_array); $i < $iMax; $i++) {
...
WHERE attach_id = ' . $attach_id_array[$i];
            ...
            if (0 != $db->sql_numrows($result)) {
                ...
                for ($j = 0; $j < $num_attach; $j++) {
                    ...
                    if (1 == (int)$attachments[$j]['thumbnail']) {
