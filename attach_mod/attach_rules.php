 ...
 *
 * $Id: attach_rules.php,v 1.10 2005/01/18 16:02:10 acydburn Exp $
 ***************************************************************************/
...
//
if ($privmsg) {
    $auth['auth_attachments'] = (ADMIN != $userdata['user_level']) ? (int)$attach_config['allow_pm_attach'] : true;
...
for ($i = 0; $i < $num_rows; $i++) {
    ...
    if ($permit) {
        ...
        $det_filesize = (int)trim($rows[$i]['max_filesize']);
        ...
        $max_filesize = (0 == $det_filesize) ? $lang['Unlimited'] : $det_filesize . ' ' . $size_lang;
...
if ($nothing) {
    $template->assign_block_vars('switch_nothing', []);
}
