 ...
 *
 * $Id: functions_search.php,v 1.8.2.14 2005/05/03 23:24:02 acydburn Exp $
 ****************************************************************************/
...
function clean_words($mode, &$entry, $stopword_list, $synonym_list)
{
    ...
    if ('post' == $mode) {
        ...
        for ($i = 0, $iMax = count($entry); $i < $iMax; $i++) {
            $entry[$i] = trim($entry[$i]);

            if ((mb_strlen($entry[$i]) < 3) || (mb_strlen($entry[$i]) > 20)) {
                $entry[$i] = '';
            }
        }
...
function split_words($entry, $mode = 'post')
{
    ...
    */
    ...
    for ($i = 0, $iMax = count($split); $i < $iMax; $i++) {
        if ('' != trim($split[$i])) {
