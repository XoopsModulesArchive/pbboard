 ...
 *
 * $Id: functions_validate.php,v 1.6.2.12 2005/06/09 19:13:05 psotfx Exp $
 ***************************************************************************/
...
WHERE LOWER(uname) = '" . mb_strtolower($username) . "'";
    ...
    if ($result = $db->sql_query($sql)) {
        if ($row = $db->sql_fetchrow($result)) {
            if (($userdata['session_logged_in'] && $row['uname'] != $userdata['uname']) || !$userdata['session_logged_in']) {
...
FROM ' . GROUPS_TABLE . "
WHERE LOWER(group_name) = '" . mb_strtolower($username) . "'";
