 ...
 *
 * $Id: bbcode.php,v 1.36.2.27 2005/06/09 20:01:18 psotfx Exp $
 ***************************************************************************/
...
function make_bbcode_uid()
{
    ...
    $uid = mb_substr($uid, 0, BBCODE_UID_LEN);
 ...
 * @param mixed $text
 * @param mixed $uid
 * @param mixed $open_tag
 * @param mixed $close_tag
 * @param mixed $close_tag_new
 * @param mixed $mark_lowest_level
 * @param mixed $func
 * @param mixed $open_regexp_replace
 * @return mixed|string
 * @return mixed|string
 */
    ...
    while ($curr_pos && ($curr_pos < mb_strlen($text))) {
        ...
        if ($curr_pos) {
            ...
            for ($i = 0; $i < $open_tag_count; $i++) {
                ...
                $possible_start = mb_substr($text, $curr_pos, mb_strpos($text, ']', $curr_pos + 1) - $curr_pos + 1);
                ...
                if (preg_match('#\[quote=\\\"#si', $possible_start, $match) && !preg_match('#\[quote=\\\"(.*?)\\\"\]#si', $possible_start)) {
                    ...
                    if ($close_pos = mb_strpos($text, '"]', $curr_pos + 9)) {
                        if (false === mb_strpos(mb_substr($text, $curr_pos + 9, $close_pos - ($curr_pos + 9)), '[quote')) {
                            $possible_start = mb_substr($text, $curr_pos, $close_pos - $curr_pos + 2);
                        }
                    }
                }
            ...
            if ($found_start) {
                ...
                if (0 == strcasecmp($close_tag, $possible_end)) {
                    ...
                    if (count($stack) > 0) {
                        ...
                        if ($mark_lowest_level && (1 == $curr_nesting_depth)) {
                            if ('[code]' == $open_tag[0]) {
                                $code_entities_match = ['#<#', '#>#', '#"#', '#:#', '#\[#', '#\]#', '#\(#', '#\)#', '#\{#', '#\}#'];
