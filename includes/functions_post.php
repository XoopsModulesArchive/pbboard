 ...
 *
 * $Id: functions_post.php,v 1.9.2.35 2005/06/09 19:35:56 psotfx Exp $
 ***************************************************************************/
...
function prepare_post($mode, $post_data, $bbcode_on, $html_on, $smilies_on, &$error_msg, &$username, &$bbcode_uid, &$subject, &$message, &$poll_title, &$poll_options, &$poll_length)
{
    ...
    if (!empty($username)) {
        ...
        if (!$userdata['session_logged_in'] || ($userdata['session_logged_in'] && $username != $userdata['uname'])) {
...
WHERE $where_sql";
    ...
    if ('newtopic' == $mode || ('editpost' == $mode && $post_data['first_post'])) {
        ...
        $sql = ('editpost' != $mode) ? 'INSERT INTO '
                                              . TOPICS_TABLE
                                              . " (topic_title, topic_poster, topic_time, forum_id, topic_status, topic_type, topic_vote) VALUES ('$post_subject', "
                                              . $userdata['uid']
                                              . ", $current_time, $forum_id, "
                                              . TOPIC_UNLOCKED
                                              . ", $topic_type, $topic_vote)" : 'UPDATE ' . TOPICS_TABLE . " SET topic_title = '$post_subject', topic_type = $topic_type " . (($post_data['edit_vote'] || !empty($poll_title)) ? ', topic_vote = ' . $topic_vote : '') . " WHERE topic_id = $topic_id";
...
AND vote_id = $poll_id";
    ...
    $message = $lang['Stored'] . '<br><br>' . sprintf($lang['Click_view_message'], '<a href="' . append_sid("viewtopic.$phpEx?" . POST_POST_URL . '=' . $post_id) . '#' . $post_id . '">', '</a>') . '<br><br>' . sprintf(
...
WHERE vote_id = $poll_id";
    ...
    if ('delete' == $mode && $post_data['first_post'] && $post_data['last_post']) {
        ...
        $message = (('poll_delete' == $mode) ? $lang['Poll_delete'] : $lang['Deleted']) . '<br><br>' . sprintf($lang['Click_return_topic'], '<a href="' . append_sid("viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id") . '">', '</a>');
    }
...
FROM ' . BANLIST_TABLE;
            ...
            $sql = 'SELECT u.uid, u.email, u.user_lang
