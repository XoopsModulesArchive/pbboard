...
AND f.forum_id = t.forum_id";
    ...
    $sql = 'SELECT u.uname, u.uid, p.*, pt.post_text, pt.post_subject, pt.bbcode_uid
...
LIMIT " . $board_config['posts_per_page'];
    ...
    if ($row = $db->sql_fetchrow($result)) {
        ...
        do {
            ...
            if (ANONYMOUS == $poster_id && '' != $row['post_username']) {
                $poster = $row['post_username'];

                $poster_rank = $lang['Guest'];
            } elseif (ANONYMOUS == $poster_id) {
                $poster = $lang['Guest'];
