 ...
 *
 * $Id: auth.php,v 1.37.2.3 2005/02/25 16:02:59 acydburn Exp $
 ***************************************************************************/
...
function auth($type, $forum_id, $userdata, $f_access = '')
{
    ...
    if (empty($f_access)) {
        $forum_match_sql = (AUTH_LIST_ALL != $forum_id) ? "WHERE a.forum_id = $forum_id" : '';
...
$forum_match_sql";
        ...
        $sql_fetchrow = (AUTH_LIST_ALL != $forum_id) ? 'sql_fetchrow' : 'sql_fetchrowset';
    ...
    if ($userdata['session_logged_in']) {
        $forum_match_sql = (AUTH_LIST_ALL != $forum_id) ? "AND a.forum_id = $forum_id" : '';
...
$forum_match_sql";
    ...
    $is_admin = (ADMIN == $userdata['user_level'] && $userdata['session_logged_in']) ? true : 0;

    $auth_user = [];

    for ($i = 0, $iMax = count($auth_fields); $i < $iMax; $i++) {
        $key = $auth_fields[$i];
        ...
        if (AUTH_LIST_ALL != $forum_id) {
            ...
            for ($k = 0, $kMax = count($f_access); $k < $kMax; $k++) {
