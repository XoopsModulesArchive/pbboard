 ...
 *
 * $Id: usercp_register.php,v 1.20.2.51 2005/06/11 10:05:15 psotfx Exp $
 ***************************************************************************/
...
if (isset($_POST['submit']) || isset($_POST['avatargallery']) || isset($_POST['submitavatar']) || isset($_POST['cancelavatar']) || $mode == 'register') {
    ...
    $user_avatar_local     = (isset($_POST['avatarselect']) && !empty($_POST['submitavatar']) && $board_config['allow_avatar_local']) ? $_POST['avatarselect'] : ((isset($_POST['avatarlocal'])) ? htmlspecialchars($_POST['avatarlocal'], ENT_QUOTES | ENT_HTML5) : '');
...
//
if ($mode == 'register' && ($userdata['session_logged_in'] || $username == $userdata['uname'])) {
...
SET '
                   ...
                   . "', user_msnm = '"
                   . str_replace("\'", "''", $msn)
                   . "', attachsig = $attachsig, user_allowsmile = $allowsmilies, user_allowhtml = $allowhtml, user_allowbbcode = $allowbbcode, user_allow_viewonline = $allowviewonline, user_notify = $notifyreply, user_notify_pm = $notifypm, user_popup_pm = $popup_pm, timezone_offset = $user_timezone, user_dateformat = '"
                   . str_replace("\'", "''", $user_dateformat)
                   . "', user_lang = '"
                   . str_replace("\'", "''", $user_lang)
                   . "', user_style = $user_style, level = $user_active, actkey = '"
