 ...
 *
 * $Id: modcp.php,v 1.71.2.20 2005/03/02 15:51:06 acydburn Exp $
 ***************************************************************************/
...
AND f.forum_id = t.forum_id';
    ...
    $forum_topics = (0 == $topic_row['forum_topics']) ? 1 : $topic_row['forum_topics'];
...
switch ($mode) {
    case 'delete':
        ...
        if ($confirm) {
            ...
            $topics = $_POST['topic_id_list'] ?? [$topic_id];

            $topic_id_sql = '';

            for ($i = 0, $iMax = count($topics); $i < $iMax; $i++) {
...
SET posts = posts - ' . $row['posts'] . '
WHERE uid = ' . $row['poster_id'];
            }

            $db->sql_freeresult($result);

            if (count($count_sql)) {
                for ($i = 0, $iMax = count($count_sql); $i < $iMax; $i++) {
...
WHERE topic_id IN ($topic_id_sql)";
            ...
            if (!empty($topic_id)) {
                $redirect_page = "viewforum.$phpEx?" . POST_FORUM_URL . "=$forum_id&amp;sid=" . $userdata['sess_id'];

                $l_redirect = sprintf($lang['Click_return_forum'], '<a href="' . $redirect_page . '">', '</a>');
            } else {
                $redirect_page = "modcp.$phpEx?" . POST_FORUM_URL . "=$forum_id&amp;sid=" . $userdata['sess_id'];
            ...
            $hidden_fields = '<input type="hidden" name="sid" value="' . $userdata['sess_id'] . '"><input type="hidden" name="mode" value="' . $mode . '"><input type="hidden" name="' . POST_FORUM_URL . '" value="' . $forum_id . '">';
...
WHERE topic_id = $topic_id";
            ...
            if (!empty($topic_id)) {
                $redirect_page = "viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;sid=" . $userdata['sess_id'];
            ...
            $message .= '<br \><br \>' . sprintf($lang['Click_return_forum'], '<a href="' . "viewforum.$phpEx?" . POST_FORUM_URL . "=$old_forum_id&amp;sid=" . $userdata['sess_id'] . '">', '</a>');
...
AND topic_moved_id = 0";
        ...
        $message .= '<br \><br \>' . sprintf($lang['Click_return_forum'], '<a href="' . "viewforum.$phpEx?" . POST_FORUM_URL . "=$forum_id&amp;sid=" . $userdata['sess_id'] . '">', '</a>');
        $template->assign_vars(
            [
                'META' => '<meta http-equiv="refresh" content="3;url=' . $redirect_page . '">',
            ]
...
ORDER BY post_time ASC";
            ...
            if ($row = $db->sql_fetchrow($result)) {
                ...
                $post_subject = trim(htmlspecialchars($_POST['subject'], ENT_QUOTES | ENT_HTML5));
...
WHERE $sql_where";
                ...
                $template->assign_vars(
                    [
                        'META' => '<meta http-equiv="refresh" content="3;url=' . "viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;sid=" . $userdata['sess_id'] . '">',
                    ]
                );

                $message = $lang['Topic_split'] . '<br><br>' . sprintf($lang['Click_return_topic'], '<a href="' . "viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;sid=" . $userdata['sess_id'] . '">', '</a>');
            ...
            $sql = 'SELECT u.uname, p.*, pt.post_text, pt.bbcode_uid, pt.post_subject, p.post_username
...
ORDER BY p.post_time ASC";
            ...
            $s_hidden_fields = '<input type="hidden" name="sid" value="'
                               . $userdata['sess_id']
...
'IP' => $ip_this_post,
'U_LOOKUP_IP' => "modcp.$phpEx?mode=ip&amp;" . POST_POST_URL . "=$post_id&amp;" . POST_TOPIC_URL . "=$topic_id&amp;rdns=$ip_this_post&amp;sid=" . $userdata['sess_id'],
            ]
        );
        //
        ...
        if ($row = $db->sql_fetchrow($result)) {
            ...
            do {
                if ($row['poster_ip'] == $post_row['poster_ip']) {
                    $template->assign_vars(
                        [
                            'POSTS' => $row['postings'] . ' ' . ((1 == $row['postings']) ? $lang['Post'] : $lang['Posts']),
                        ]
                    );
                ...
                $ip = ($rdns_ip_num == $row['poster_ip'] || 'all' == $rdns_ip_num) ? gethostbyaddr($ip) : $ip;
...
'IP' => $ip,
'POSTS' => $row['postings'] . ' ' . ((1 == $row['postings']) ? $lang['Post'] : $lang['Posts']),
'U_LOOKUP_IP' => "modcp.$phpEx?mode=ip&amp;" . POST_POST_URL . "=$post_id&amp;" . POST_TOPIC_URL . "=$topic_id&amp;rdns=" . $row['poster_ip'] . '&amp;sid=' . $userdata['sess_id'],
                    ]
                );
        ...
        //
        $sql = 'SELECT u.uid, u.uname, COUNT(*) as postings
...
ORDER BY " . ((SQL_LAYER == 'msaccess') ? 'COUNT(*)' : 'postings') . ' DESC';
        ...
        if ($row = $db->sql_fetchrow($result)) {
            ...
            do {
                ...
                $username = (ANONYMOUS == $id) ? $lang['Guest'] : $row['uname'];
...
'U_PROFILE' => (ANONYMOUS == $id) ? "modcp.$phpEx?mode=ip&amp;" . POST_POST_URL . '=' . $post_id . '&amp;' . POST_TOPIC_URL . '=' . $topic_id . '&amp;sid=' . $userdata['sess_id'] : append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . "=$id"),
...
'L_SELECT' => $lang['Select'],
'U_VIEW_FORUM' => append_sid("viewforum.$phpEx?" . POST_FORUM_URL . "=$forum_id"),
'S_HIDDEN_FIELDS' => '<input type="hidden" name="sid" value="' . $userdata['sess_id'] . '"><input type="hidden" name="' . POST_FORUM_URL . '" value="' . $forum_id . '">',
'S_MODCP_ACTION' => append_sid("modcp.$phpEx"),
            ]
        );
        $template->set_filenames(
        ...
        $replacement_word = [];
        obtain_word_list($orig_word, $replacement_word);
        $sql = 'SELECT t.*, u.uname, u.uid, p.post_time
...
LIMIT $start, " . $board_config['topics_per_page'];
        ...
        while (false !== ($row = $db->sql_fetchrow($result))) {
            ...
            } elseif (TOPIC_MOVED == $topic_status) {
            ...
            $u_view_topic = "modcp.$phpEx?mode=split&amp;" . POST_TOPIC_URL . "=$topic_id&amp;sid=" . $userdata['sess_id'];
...
'L_TOPIC_FOLDER_ALT' => $folder_alt,
        ...
        $template->assign_vars(
            [
                'PAGINATION' => generate_pagination("modcp.$phpEx?" . POST_FORUM_URL . "=$forum_id&amp;sid=" . $userdata['sess_id'], $forum_topics, $board_config['topics_per_page'], $start),
