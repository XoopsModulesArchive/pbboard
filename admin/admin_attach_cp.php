 ...
 *
 * $Id: admin_attach_cp.php,v 1.16 2005/03/07 17:48:23 acydburn Exp $
 ***************************************************************************/
...
// Pagination ?
//
$do_pagination = (('stats' != $view) && ('search' != $view)) ? true : false;
//
// Set Order
//
$order_by = '';
if ('username' == $view) {
    switch ($mode) {
        case 'username':
            $order_by = 'ORDER BY u.uname ' . $sort_order . ' LIMIT ' . $start . ', ' . $board_config['topics_per_page'];
            break;
        case 'attachments':
...
if ($submit_change && 'attachments' == $view) {
    $attach_change_list = $_POST['attach_id_list'] ?? [];

    $attach_comment_list = $_POST['attach_comment_list'] ?? [];

    $attach_download_count_list = $_POST['attach_count_list'] ?? [];
    ...
    for ($i = 0, $iMax = count($attach_change_list); $i < $iMax; $i++) {
        $attachments['_' . $attach_change_list[$i]]['comment'] = stripslashes(htmlspecialchars($attach_comment_list[$i], ENT_QUOTES | ENT_HTML5));

        $attachments['_' . $attach_change_list[$i]]['download_count'] = (int)$attach_download_count_list[$i];
    }
...
ORDER BY attach_id';
    ...
    while (false !== ($attachrow = $db->sql_fetchrow($result))) {
        if (isset($attachments['_' . $attachrow['attach_id']])) {
            if (($attachrow['comment'] != $attachments['_' . $attachrow['attach_id']]['comment']) || ((int)$attachrow['download_count'] != (int)$attachments['_' . $attachrow['attach_id']]['download_count'])) {
                $sql = 'UPDATE ' . ATTACHMENTS_DESC_TABLE . " 
SET comment = '" . $attachments['_' . $attachrow['attach_id']]['comment'] . "', download_count = " . (int)$attachments['_' . $attachrow['attach_id']]['download_count'] . '
...
if ('stats' == $view) {
    ...
    if ((int)$attach_config['attachment_quota'] >= 1048576) {
        $attachment_quota = round((int)$attach_config['attachment_quota'] / 1048576 * 100) / 100 . ' ' . $lang['MB'];
    } elseif ((int)$attach_config['attachment_quota'] >= 1024) {
        $attachment_quota = round((int)$attach_config['attachment_quota'] / 1024 * 100) / 100 . ' ' . $lang['KB'];
    } else {
        $attachment_quota = (int)$attach_config['attachment_quota'] . ' ' . $lang['Bytes'];
    }
...
'S_ORDER_SELECT' => $select_sort_order,
    ...
    $sql = 'SELECT u.uname, a.user_id_1 as uid, count(*) as total_attachments
FROM ' . ATTACHMENTS_TABLE . ' a, ' . USERS_TABLE . ' u
WHERE (a.user_id_1 = u.uid)
GROUP BY a.user_id_1, u.uname';
...
'U_VIEW_MEMBER' => append_sid('admin_attach_cp.' . $phpEx . '?view=attachments&amp;uid=' . $members[$i]['uid']),
                ]
            );
        }
    }
