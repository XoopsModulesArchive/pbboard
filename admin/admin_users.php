 ...
 *
 * $Id: admin_users.php,v 1.57.2.24 2005/05/13 18:57:45 acydburn Exp $
 ***************************************************************************/
...
WHERE group_moderator = $user_id";
            ...
            while (false !== ($row_group = $db->sql_fetchrow($result))) {
...
SET group_moderator = ' . $userdata['uid'] . "
    ...
    if (isset($_POST['submit'])) {
        ...
        if (stripslashes($username) != $this_userdata['uname']) {
            ...
            if (stripslashes(mb_strtolower($username)) != mb_strtolower($this_userdata['uname'])) {
                ...
                } elseif (mb_strtolower(str_replace("\'", "''", $username)) == mb_strtolower($userdata['uname'])) {
                    $error = true;
...
SET '
                   ...
                   . "', user_style = $user_style, timezone_offset = $user_timezone, user_dateformat = '"
                   . str_replace("\'", "''", $user_dateformat)
                   . "', level = $user_status, rank = $user_rank"
                   . $avatar_sql
                   . "
WHERE uid = $user_id";
...
WHERE group_name = '" . str_replace("'", "''", $this_userdata['uname']) . "'";
