 ...
 *
 * $Id: admin_db_utilities.php,v 1.42.2.10 2005/03/04 21:02:19 acydburn Exp $
 ****************************************************************************/
...
if (!empty($setmodules)) {
    ...
    if ((empty($file_uploads) || 0 != $file_uploads) && ('off' != mb_strtolower($file_uploads)) && ('4.0.4pl1' != @phpversion())) {
...
AND relkind = 'S' ORDER BY relname";
    ...
    if (!$num_seq = $db->sql_numrows($seq)) {
        ...
        while ($i_seq < $num_seq) {
            ...
            if (($row['last_value'] > 1) && ('structure' != $backup_type)) {
...
ORDER BY index_name, tab_name, column_name ";
    ...
    while (false !== ($row = $db->sql_fetchrow($result))) {
        if ('t' == $row['primary_key']) {
            ...
            $index_rows[$row['index_name']]['unique'] = ('t' == $row['unique_key']) ? ' UNIQUE ' : '';
...
if (isset($_GET['perform']) || isset($_POST['perform'])) {
    $perform = $_POST['perform'] ?? $_GET['perform'];
...
'MESSAGE_TEXT' => $lang['Backups_not_supported'],
            ...
            $additional_tables = $_POST['additional_tables'] ?? ($_GET['additional_tables'] ?? '');
            $backup_type = $_POST['backup_type'] ?? ($_GET['backup_type'] ?? '');
            ...
            if (!empty($additional_tables)) {
                if (preg_match(',', $additional_tables)) {
                    ...
                    for ($i = 0, $iMax = count($additional_tables); $i < $iMax; $i++) {
                ...
                $template->assign_vars(
                    [
                        'META' => '<meta http-equiv="refresh" content="2;url='
                                           . append_sid("admin_db_utilities.$phpEx?perform=backup&additional_tables=" . quotemeta($additional_tables) . "&backup_type=$backup_type&drop=1&amp;backupstart=1&gzipcompress=$gzipcompress&startdownload=1")
                                           . '">',
...
'MESSAGE_TEXT' => $lang['Backup_download'],
            ...
            if ($do_gzip_compress) {
                ...
                echo "\x1f\x8b\x08\x00\x00\x00\x00\x00" . mb_substr($contents, 0, -4) . gzip_PrintFourChars($Crc) . gzip_PrintFourChars($Size);
            }
            exit;
            break;
        case 'restore':
...
'S_HIDDEN_FIELDS' => $s_hidden_fields,
                ...
                $backup_file_tmpname = ('none' != $HTTP_POST_FILES['backup_file']['tmp_name']) ? $HTTP_POST_FILES['backup_file']['tmp_name'] : '';
                ...
                if (file_exists(phpbb_realpath($backup_file_tmpname))) {
                    if (preg_match("/^(text\/[a-zA-Z]+)|(application\/(x\-)?gzip(\-compressed)?)|(application\/octet-stream)$/is", $backup_file_type)) {
                        if (preg_match("/\.gz$/is", $backup_file_name)) {
                            ...
                            $sql_query = fread(fopen($backup_file_tmpname, 'rb'), filesize($backup_file_tmpname));
                        }
