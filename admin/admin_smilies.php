...
while ($file = @readdir($dir)) {
    if (!@is_dir(phpbb_realpath($phpbb_root_path . $board_config['smilies_path'] . '/' . $file))) {
        ...
        if ($img_size[0] && $img_size[1] && preg_match('/smil*/', $file)) {
            $smiley_images[] = $file;
...
if (isset($_GET['import_pack']) || isset($_POST['import_pack'])) {
    ...
    $smile_pak = $_POST['smile_pak'] ?? $_GET['smile_pak'];

    $clear_current = $_POST['clear_current'] ?? $_GET['clear_current'];

    $replace_existing = $_POST['replace'] ?? $_GET['replace'];
...
SET smile_url = '" . str_replace("\'", "''", $smile_data[0]) . "', emotion = '" . str_replace("\'", "''", $smile_data[1]) . "'
WHERE code = '" . str_replace("\'", "''", $smile_data[$j]) . "'";
                    } else {
                        $sql = '';
                    }
                } else {
                    $sql = 'INSERT INTO ' . SMILIES_TABLE . " (code, smile_url, emotion)
VALUES('" . str_replace("\'", "''", $smile_data[$j]) . "', '" . str_replace("\'", "''", $smile_data[0]) . "', '" . str_replace("\'", "''", $smile_data[1]) . "')";
        ...
        $message = $lang['smiley_import_success'] . '<br><br>' . sprintf($lang['Click_return_smileadmin'], '<a href="' . append_sid("admin_smilies.$phpEx") . '">', '</a>') . '<br><br>' . sprintf(
...
FROM ' . SMILIES_TABLE;
        ...
        for ($i = 0, $iMax = count($resultset); $i < $iMax; $i++) {
            ...
            $smile_pak .= $resultset[$i]['emotion'] . $delimeter;
    ...
    $message = sprintf($lang['export_smiles'], '<a href="' . append_sid("admin_smilies.$phpEx?export_pack=send", true) . '">', '</a>') . '<br><br>' . sprintf($lang['Click_return_smileadmin'], '<a href="' . append_sid("admin_smilies.$phpEx") . '">', '</a>') . '<br><br>' . sprintf(
        $lang['Click_return_admin_index'],
        '<a href="' . append_sid("index.$phpEx?pane=right") . '">',
        '</a>'
    );
...
WHERE id = ' . $smiley_id;
            ...
            $message = $lang['smiley_del_success'] . '<br><br>' . sprintf($lang['Click_return_smileadmin'], '<a href="' . append_sid("admin_smilies.$phpEx") . '">', '</a>') . '<br><br>' . sprintf(
                $lang['Click_return_admin_index'],
                '<a href="' . append_sid("index.$phpEx?pane=right") . '">',
                '</a>'
            );
...
WHERE id = ' . $smiley_id;
            ...
            $s_hidden_fields = '<input type="hidden" name="mode" value="save"><input type="hidden" name="smile_id" value="' . $smile_data['id'] . '">';
            $template->assign_vars(
                [
                    'SMILEY_CODE' => $smile_data['code'],
'SMILEY_EMOTICON' => $smile_data['emotion'],
'L_SMILEY_TITLE' => $lang['smiley_title'],
'L_SMILEY_CONFIG' => $lang['smiley_config'],
'L_SMILEY_EXPLAIN' => $lang['smile_desc'],
'L_SMILEY_CODE' => $lang['smiley_code'],
...
'S_SMILEY_BASEDIR' => $phpbb_root_path . $board_config['smilies_path'],
        ...
        case 'save':
            ...
            //
            $sql = 'UPDATE ' . SMILIES_TABLE . "
SET code = '" . str_replace("\'", "''", $smile_code) . "', smile_url = '" . str_replace("\'", "''", $smile_url) . "', emotion = '" . str_replace("\'", "''", $smile_emotion) . "'
WHERE id = $smile_id";
            if (!($result = $db->sql_query($sql))) {
                message_die(GENERAL_ERROR, "Couldn't update smilies info", '', __LINE__, __FILE__, $sql);
            }
            $message = $lang['smiley_edit_success'] . '<br><br>' . sprintf($lang['Click_return_smileadmin'], '<a href="' . append_sid("admin_smilies.$phpEx") . '">', '</a>') . '<br><br>' . sprintf(
                $lang['Click_return_admin_index'],
                '<a href="' . append_sid("index.$phpEx?pane=right") . '">',
                '</a>'
            );
...
VALUES ('" . str_replace("\'", "''", $smile_code) . "', '" . str_replace("\'", "''", $smile_url) . "', '" . str_replace("\'", "''", $smile_emotion) . "')";
            ...
            $message = $lang['smiley_add_success'] . '<br><br>' . sprintf($lang['Click_return_smileadmin'], '<a href="' . append_sid("admin_smilies.$phpEx") . '">', '</a>') . '<br><br>' . sprintf(
                $lang['Click_return_admin_index'],
                '<a href="' . append_sid("index.$phpEx?pane=right") . '">',
                '</a>'
            );
...
'U_SMILEY_EDIT' => append_sid("admin_smilies.$phpEx?mode=edit&amp;id=" . $smilies[$i]['id']),
'U_SMILEY_DELETE' => append_sid("admin_smilies.$phpEx?mode=delete&amp;id=" . $smilies[$i]['id']),
            ]
        );
    }
