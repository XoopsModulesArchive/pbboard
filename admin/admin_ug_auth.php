 ...
 *
 * $Id: admin_ug_auth.php,v 1.13.2.4 2005/05/17 17:32:19 acydburn Exp $
 ***************************************************************************/
...
// End Functions
// -------------
if (isset($_POST['submit']) && (('user' == $mode && $user_id) || ('group' == $mode && $group_id))) {
    $user_level = '';
...
WHERE group_id = $group_id";
        ...
        $message = $lang['Auth_updated'] . '<br><br>' . sprintf($lang['Click_return_userauth'], '<a href="' . append_sid("admin_ug_auth.$phpEx?mode=$mode") . '">', '</a>') . '<br><br>' . sprintf(
...
ORDER BY forum_order';
            ...
            $sql = ('user' == $mode) ? 'SELECT aa.* FROM ' . AUTH_ACCESS_TABLE . ' aa, ' . USER_GROUP_TABLE . ' ug, ' . GROUPS_TABLE . " g WHERE ug.user_id = $user_id AND g.group_id = ug.group_id AND aa.group_id = ug.group_id AND g.group_single_user = " . true : 'SELECT * FROM '
                                                                                                                                                                                                                                                                       . AUTH_ACCESS_TABLE
                                                                                                                                                                                                                                                                       . " WHERE group_id = $group_id";
...
} elseif (('user' == $mode && (isset($_POST['username']) || $user_id)) || ('group' == $mode && $group_id)) {
...
ORDER BY forum_order';
    ...
    $sql = 'SELECT u.uid, u.uname, u.user_level, g.group_id, g.group_name, g.group_single_user FROM ' . USERS_TABLE . ' u, ' . GROUPS_TABLE . ' g, ' . USER_GROUP_TABLE . ' ug WHERE ';

    $sql .= ('user' == $mode) ? "u.uid = $user_id AND ug.user_id = u.uid AND g.group_id = ug.group_id" : "g.group_id = $group_id AND ug.group_id = g.group_id AND u.uid = ug.user_id";
    ...
    $sql = ('user' == $mode) ? 'SELECT aa.*, g.group_single_user FROM ' . AUTH_ACCESS_TABLE . ' aa, ' . USER_GROUP_TABLE . ' ug, ' . GROUPS_TABLE . " g WHERE ug.user_id = $user_id AND g.group_id = ug.group_id AND aa.group_id = ug.group_id AND g.group_single_user = 1" : 'SELECT * FROM '
                                                                                                                                                                                                                                                                              . AUTH_ACCESS_TABLE
                                                                                                                                                                                                                                                                              . " WHERE group_id = $group_id";
    ...
    $is_admin = ('user' == $mode) ? ((ADMIN == $ug_info[0]['user_level'] && ANONYMOUS != $ug_info[0]['uid']) ? 1 : 0) : 0;
...
'S_MOD_SELECT' => $optionlist_mod,
    ...
    for ($i = 0, $iMax = count($ug_info); $i < $iMax; $i++) {
        if (('user' == $mode && !$ug_info[$i]['group_single_user']) || 'group' == $mode) {
            $name[] = ('user' == $mode) ? $ug_info[$i]['group_name'] : $ug_info[$i]['username'];

            $id[] = ('user' == $mode) ? (int)$ug_info[$i]['group_id'] : (int)$ug_info[$i]['user_id'];
        }
    }
    ...
    $u_ug_switch = ('user' == $mode) ? POST_USERS_URL . '=' . $user_id : POST_GROUPS_URL . '=' . $group_id;
    ...
    $s_hidden_fields .= ('user' == $mode) ? '<input type="hidden" name="' . POST_USERS_URL . '" value="' . $user_id . '">' : '<input type="hidden" name="' . POST_GROUPS_URL . '" value="' . $group_id . '">';
