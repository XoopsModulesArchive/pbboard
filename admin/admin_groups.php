 ...
 *
 * $Id: admin_groups.php,v 1.25.2.8 2005/05/17 17:32:18 acydburn Exp $
 ***************************************************************************/
...
ORDER BY uname';
    ...
    while (false !== ($row = $db->sql_fetchrow($result))) {
        if ($row['uid'] == $group_info['group_moderator']) {
            $group_moderator = $row['uname'];
        }
    }
    $group_open      = ($group_info['group_type'] == GROUP_OPEN) ? ' checked' : '';
    $group_closed    = ($group_info['group_type'] == GROUP_CLOSED) ? ' checked' : '';
    $group_hidden    = ($group_info['group_type'] == GROUP_HIDDEN) ? ' checked' : '';
...
WHERE user_level = ' . MOD . ' AND uid = ' . (int)$rows[$i]['user_id'];
                    if (!$db->sql_query($sql)) {
...
WHERE group_id = ' . $group_id;
        ...
        $message = $lang['Deleted_group'] . '<br><br>'set($_POST['group_name']) ? trim($_POST['group_name']) : '';
...
$group_moderator = $this_userdata['uid'];
if (!$group_moderator) {
    message_die(GENERAL_MESSAGE, $lang['No_group_moderator']);
}
if ($mode == 'editgroup') {
