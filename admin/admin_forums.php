 ...
 *
 * $Id: admin_forums.php,v 1.40.2.10 2005/01/05 02:36:00 psotfx Exp $
 ***************************************************************************/
...
WHERE forum_id = $forum_id";
            ...
            (FORUM_LOCKED) == $forumstatus ? $forumlocked = 'selected="selected"' : $forumunlocked = 'selected="selected"';
            // These two options ($lang['Status_unlocked'] and $lang['Status_locked']) seem to be missing from
            // the language files.
            $lang['Status_unlocked'] = $lang['Status_unlocked'] ?? 'Unlocked';
            $lang['Status_locked'] = $lang['Status_locked'] ?? 'Locked';
...
VALUES('" . $next_id . "', " . (int)$_POST['prune_days'] . ', ' . (int)$_POST['prune_freq'] . ')';
            ...
            $message = $lang['Forums_updated'] . '<br><br>' . sprintf($lang['Click_return_forumadmin'], '<a href="' . append_sid("admin_forums.$phpEx") . '">', '</a>') . '<br><br>' . sprintf(
...
SET forum_name = '"
                   ...
                   . "', forum_status = "
                   . (int)$_POST['forumstatus']
                   . ', prune_enable = '
                   . (int)$_POST['prune_enable']
                   . '
WHERE forum_id = '
                   . (int)$_POST[POST_FORUM_URL];
            if (!$result = $db->sql_query($sql)) {
...
WHERE forum_id = $from_id";
            ...
            // Alter Mod level if appropriate - 2.0.7
...
AND ug.user_id NOT IN ($user_ids)";
                ...
                if ($row = $db->sql_fetchrow($result2)) {
                    ...
                    do {
                        $user_ids .= (('' != $user_ids) ? ', ' : '') . $row['user_id'];
                    } while (false !== ($row = $db->sql_fetchrow($result2)));
