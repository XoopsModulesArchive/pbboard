...
if (!empty($setmodules)) {
    ...
    $module['Styles']['Manage'] = (string)$file;
...
switch ($mode) {
    case 'addnew':
        ...
        if (isset($install_to)) {
            ...
            for ($i = 0, $iMax = count($db_fields); $i < $iMax; $i++) {
                $sql .= $db_fields[$i];
            ...
            for ($i = 0, $iMax = count($db_values); $i < $iMax; $i++) {
                $sql .= "'" . $db_values[$i] . "'";
            ...
            $message = $lang['Theme_installed'] . '<br><br>' . sprintf($lang['Click_return_styleadmin'], '<a href="' . append_sid("admin_styles.$phpEx") . '">', '</a>') . '<br><br>' . sprintf(
            ...
            if ($dir = @opendir($phpbb_root_path . 'templates/')) {
                while ($sub_dir = @readdir($dir)) {
                    if (!is_file(phpbb_realpath($phpbb_root_path . 'templates/' . $sub_dir)) && !is_link(phpbb_realpath($phpbb_root_path . 'templates/' . $sub_dir)) && '.' != $sub_dir && '..' != $sub_dir && 'CVS' != $sub_dir) {
                        if (@file_exists(@phpbb_realpath($phpbb_root_path . 'templates/' . $sub_dir . '/theme_info.cfg'))) {
                            ...
                            for ($i = 0, $iMax = count($$sub_dir); $i < $iMax; $i++) {
...
'L_ACTION' => $lang['Action'],
                ...
                for ($i = 0, $iMax = count($installable_themes); $i < $iMax; $i++) {
        ...
        if ($submit) {
            ...
            if ('edit' == $mode) {
                ...
                while (list($key, $val) = each($updated)) {
                    ...
                    $sql .= (mb_stristr($key, 'fontsize')) ? "$key = $val" : "$key = '" . str_replace("\'", "''", $val) . "'";
...
SET ';
                    ...
                    for ($i = 0, $iMax = count($vals); $i < $iMax; $i++) {
                        if ($i > 0) {
                            $sql .= ', ';
                        }
                ...
                $message = $lang['Theme_updated'] . '<br><br>' . sprintf($lang['Click_return_styleadmin'], '<a href="' . append_sid("admin_styles.$phpEx") . '">', '</a>') . '<br><br>' . sprintf(
                    $lang['Click_return_admin_index'],
                    '<a href="' . append_sid("index.$phpEx?pane=right") . '">',
                    '</a>'
                );
...
INTO ' . THEMES_TABLE . ' (';
                ...
                for ($i = 0, $iMax = count($field_names); $i < $iMax; $i++) {
                    if (0 != $i) {
                        $sql .= ', ';
                    }
                ...
                $message = $lang['Theme_created'] . '<br><br>' . sprintf($lang['Click_return_styleadmin'], '<a href="' . append_sid("admin_styles.$phpEx") . '">', '</a>') . '<br><br>' . sprintf(
                    $lang['Click_return_admin_index'],
                    '<a href="' . append_sid("index.$phpEx?pane=right") . '">',
                    '</a>'
                );
...
WHERE themes_id = $style_id";
            ...
            if ($dir = @opendir($phpbb_root_path . 'templates/')) {
                ...
                while ($file = @readdir($dir)) {
                    if (!is_file(phpbb_realpath($phpbb_root_path . 'templates/' . $file)) && !is_link(phpbb_realpath($phpbb_root_path . 'templates/' . $file)) && '.' != $file && '..' != $file && 'CVS' != $file) {
...
WHERE template_name = '$template_name'";
            ...
            for ($i = 0, $iMax = count($theme_rowset); $i < $iMax; $i++) {
                while (list($key, $val) = each($theme_rowset[$i])) {
                    if (!(int)$key && '0' != $key && 'themes_id' != $key) {
            ...
            $fp = @fopen($phpbb_root_path . 'templates/' . $template_name . '/theme_info.cfg', 'wb');
            ...
            if (!$fp) {
                ...
                $s_hidden_fields = '<input type="hidden" name="theme_info" value="' . htmlspecialchars($theme_data, ENT_QUOTES | ENT_HTML5) . '">';
...
'MESSAGE_TEXT' => $lang['Download_theme_cfg'] . '<br><br>' . $download_form,
            ...
            $message = $lang['Theme_info_saved'] . '<br><br>' . sprintf($lang['Click_return_styleadmin'], '<a href="' . append_sid("admin_styles.$phpEx") . '">', '</a>') . '<br><br>' . sprintf(
                $lang['Click_return_admin_index'],
                '<a href="' . append_sid("index.$phpEx?pane=right") . '">',
                '</a>'
            );
            ...
            if ($dir = @opendir($phpbb_root_path . 'templates/')) {
                ...
                while ($file = @readdir($dir)) {
                    if (!is_file(phpbb_realpath($phpbb_root_path . 'templates/' . $file)) && !is_link(phpbb_realpath($phpbb_root_path . 'templates/' . $file)) && '.' != $file && '..' != $file && 'CVS' != $file && 'blocks' != $file) {
                        $s_template_select .= '<option value="' . $file . '">' . $file . "</option>\n";
                    }
                }
...
WHERE user_style = $style_id";
            ...
            $message = $lang['Style_removed'] . '<br><br>' . sprintf($lang['Click_return_styleadmin'], '<a href="' . append_sid("admin_styles.$phpEx") . '">', '</a>') . '<br><br>' . sprintf(
                $lang['Click_return_admin_index'],
                '<a href="' . append_sid("index.$phpEx?pane=right") . '">',
                '</a>'
            );
...
'L_DELETE' => $lang['Delete'],
        ...
        for ($i = 0, $iMax = count($style_rowset); $i < $iMax; $i++) {
            $row_color = (!($i % 2)) ? $theme['td_color1'] : $theme['td_color2'];
