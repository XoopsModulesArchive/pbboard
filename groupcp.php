 ...
 *
 * $Id: groupcp.php,v 1.58.2.18 2005/06/10 17:18:13 acydburn Exp $
 ***************************************************************************/
...
function generate_user_info($row, $date_format, $group_mod, &$from, &$posts, &$joined, &$poster_avatar, &$profile_img, &$profile, &$search_img, &$search, &$pm_img, &$pm, &$email_img, &$email, &$www_img, &$www, &$icq_status_img, &$icq_img, &$icq, &$aim_img, &$aim, &$msn_img, &$msn, &$yim_img, &$yim)
{
    ...
    if ($row['user_avatar_type'] && ANONYMOUS != $row['uid'] && $row['user_allowavatar']) {
    ...
    if (!empty($row['user_viewemail']) || $group_mod) {
        $email_uri = ($board_config['board_email_form']) ? append_sid("profile.$phpEx?mode=email&amp;" . POST_USERS_URL . '=' . $row['uid']) : 'mailto:' . $row['email'];
    ...
    $temp_url = append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . '=' . $row['uid']);

    $profile_img = '<a href="' . $temp_url . '"><img src="' . $images['icon_profile'] . '" alt="' . $lang['Read_profile'] . '" title="' . $lang['Read_profile'] . '" border="0"></a>';

    $profile = '<a href="' . $temp_url . '">' . $lang['Read_profile'] . '</a>';

    $temp_url = append_sid("privmsg.$phpEx?mode=post&amp;" . POST_USERS_URL . '=' . $row['uid']);
...
WHERE group_id = $group_id";
    ...
    if ($row['group_moderator'] != $userdata['uid'] && ADMIN != $userdata['user_level']) {
        ...
        $message = $lang['Not_group_moderator'] . '<br><br>' . sprintf($lang['Click_return_group'], '<a href="' . append_sid("groupcp.$phpEx?" . POST_GROUPS_URL . "=$group_id") . '">', '</a>') . '<br><br>' . sprintf(
...
WHERE group_id = $group_id";
    ...
    $message = $lang['Group_type_updated'] . '<br><br>' . sprintf($lang['Click_return_group'], '<a href="' . append_sid("groupcp.$phpEx?" . POST_GROUPS_URL . "=$group_id") . '">', '</a>') . '<br><br>' . sprintf(
        $lang['Click_return_index'],
        '<a href="' . append_sid("index.$phpEx") . '">',
        '</a>'
    );
        ...
        if (GROUP_OPEN == $row['group_type']) {
            do {
                if ($userdata['uid'] == $row['user_id']) {
                    ...
                    $message = $lang['Already_member_group'] . '<br><br>' . sprintf($lang['Click_return_group'], '<a href="' . append_sid("groupcp.$phpEx?" . POST_GROUPS_URL . "=$group_id") . '">', '</a>') . '<br><br>' . sprintf(
                        $lang['Click_return_index'],
                        '<a href="' . append_sid("index.$phpEx") . '">',
                        '</a>'
                    );
            ...
            $message = $lang['This_closed_group'] . '<br><br>' . sprintf($lang['Click_return_group'], '<a href="' . append_sid("groupcp.$phpEx?" . POST_GROUPS_URL . "=$group_id") . '">', '</a>') . '<br><br>' . sprintf(
                $lang['Click_return_index'],
                '<a href="' . append_sid("index.$phpEx") . '">',
                '</a>'
            );
...
VALUES ($group_id, " . $userdata['uid'] . ', 1)';

    if (!($result = $db->sql_query($sql))) {
        message_die(GENERAL_ERROR, 'Error inserting user group subscription', '', __LINE__, __FILE__, $sql);
    }

    $sql = 'SELECT u.email, u.uname, u.user_lang, g.group_name
FROM ' . USERS_TABLE . ' u, ' . GROUPS_TABLE . " g
WHERE u.uid = g.group_moderator
AND g.group_id = $group_id";
    ...
    $emailer->email_address($moderator['email']);
...
'GROUP_MODERATOR' => $moderator['uname'],
...
AND group_id = $group_id";
        ...
        if (ADMIN != $userdata['user_level'] && MOD == $userdata['user_level']) {
            $sql = 'SELECT COUNT(auth_mod) AS is_auth_mod
FROM ' . AUTH_ACCESS_TABLE . ' aa, ' . USER_GROUP_TABLE . ' ug
WHERE ug.user_id = ' . $userdata['uid'] . '
...
WHERE uid = ' . $userdata['uid'];
        ...
        $message = $lang['Unsub_success'] . '<br><br>' . sprintf($lang['Click_return_group'], '<a href="' . append_sid("groupcp.$phpEx?" . POST_GROUPS_URL . "=$group_id") . '">', '</a>') . '<br><br>' . sprintf(
            $lang['Click_return_index'],
            '<a href="' . append_sid("index.$phpEx") . '">',
            '</a>'
        );
...
WHERE g.group_id = $group_id";
    ...
    if ($group_info = $db->sql_fetchrow($result)) {
        ...
        if ($group_moderator == $userdata['uid'] || ADMIN == $userdata['user_level']) {
            $is_moderator = true;
        }
        ...
        if (!empty($_POST['add']) || !empty($_POST['remove']) || isset($_POST['approve']) || isset($_POST['deny'])) {
            ...
            if (isset($_POST['add'])) {
                $username = (isset($_POST['username'])) ? htmlspecialchars($_POST['username'], ENT_QUOTES | ENT_HTML5) : '';

                $sql = 'SELECT uid, email, user_lang, user_level
FROM ' . USERS_TABLE . "
WHERE uname = '" . str_replace("\'", "''", $username) . "'";
                ...
                if (!($row = $db->sql_fetchrow($result))) {
                    ...
                    $message = $lang['Could_not_add_user'] . '<br><br>' . sprintf($lang['Click_return_group'], '<a href="' . append_sid("groupcp.$phpEx?" . POST_GROUPS_URL . "=$group_id") . '">', '</a>') . '<br><br>' . sprintf(
                        $lang['Click_return_index'],
                        '<a href="' . append_sid("index.$phpEx") . '">',
                        '</a>'
                    );
                ...
                if (ANONYMOUS == $row['uid']) {
                    ...
                    $message = $lang['Could_not_anon_user'] . '<br><br>' . sprintf($lang['Click_return_group'], '<a href="' . append_sid("groupcp.$phpEx?" . POST_GROUPS_URL . "=$group_id") . '">', '</a>') . '<br><br>' . sprintf(
                        $lang['Click_return_index'],
                        '<a href="' . append_sid("index.$phpEx") . '">',
                        '</a>'
                    );
...
VALUES (' . $row['uid'] . ", $group_id, 0)";

                    if (!$db->sql_query($sql)) {
                        message_die(GENERAL_ERROR, 'Could not add user to group', '', __LINE__, __FILE__, $sql);
                    }

                    if (ADMIN != $row['user_level'] && MOD != $row['user_level'] && $group_info['auth_mod']) {
                        $sql = 'UPDATE ' . USERS_TABLE . '
SET user_level = ' . MOD . '
...
'U_GROUPCP' => $server_url . '?' . POST_GROUPS_URL . "=$group_id",
                    ...
                    $message = $lang['User_is_member_group'] . '<br><br>' . sprintf($lang['Click_return_group'], '<a href="' . append_sid("groupcp.$phpEx?" . POST_GROUPS_URL . "=$group_id") . '">', '</a>') . '<br><br>' . sprintf(
                        $lang['Click_return_index'],
                        '<a href="' . append_sid("index.$phpEx") . '">',
                        '</a>'
                    );
...
AND group_single_user = 0";
    ...
    $sql = 'SELECT uname, uid, user_viewemail, posts, user_regdate, user_from, url, email, user_icq, user_aim, user_yim, user_msnm
FROM ' . USERS_TABLE . '
WHERE uid = ' . $group_info['group_moderator'];
    ...
    $sql = 'SELECT u.uname, u.uid, u.user_viewemail, u.posts, u.user_regdate, u.user_from, u.url, u.email, u.user_icq, u.user_aim, u.user_yim, u.user_msnm, ug.user_pending
FROM ' . USERS_TABLE . ' u, ' . USER_GROUP_TABLE . " ug
WHERE ug.group_id = $group_id
AND u.uid = ug.user_id
AND ug.user_pending = 0
AND ug.user_id <> " . $group_moderator['uid'] . '
ORDER BY u.uname';
    ...
    $sql = 'SELECT u.uname, u.uid, u.user_viewemail, u.posts, u.user_regdate, u.user_from, u.url, u.email, u.user_icq, u.user_aim, u.user_yim, u.user_msnm
...
ORDER BY u.uname";
    ...
    if ($members_count) {
        for ($i = 0; $i < $members_count; $i++) {
            if ($group_members[$i]['uid'] == $userdata['uid'] && $userdata['session_logged_in']) {
    ...
    if ($modgroup_pending_count) {
        for ($i = 0; $i < $modgroup_pending_count; $i++) {
            if ($modgroup_pending_list[$i]['uid'] == $userdata['uid'] && $userdata['session_logged_in']) {
    ...
    if ($userdata['uid'] == $group_info['group_moderator']) {
        $is_moderator = true;
...
'pendinginfo' => 'groupcp_pending_info.tpl',
    ...
    $username = $group_moderator['uname'];

    $user_id = $group_moderator['uid'];
...
'S_GROUP_OPEN_CHECKED' => (GROUP_OPEN == $group_info['group_type']) ? ' checked' : '',
'S_GROUP_CLOSED_CHECKED' => (GROUP_CLOSED == $group_info['group_type']) ? ' checked' : '',
'S_GROUP_HIDDEN_CHECKED' => (GROUP_HIDDEN == $group_info['group_type']) ? ' checked' : '',
'S_HIDDEN_FIELDS' => $s_hidden_fields,
...
'S_GROUPCP_ACTION' => append_sid("groupcp.$phpEx?" . POST_GROUPS_URL . "=$group_id"),
    ...
    for ($i = $start; $i < min($board_config['topics_per_page'] + $start, $members_count); $i++) {
        $username = $group_members[$i]['uname'];

        $user_id = $group_members[$i]['uid'];
        ...
        if (GROUP_HIDDEN != $group_info['group_type'] || $is_group_member || $is_moderator) {
...
'L_GOTO_PAGE' => $lang['Goto_page'],
    ...
    if ($is_moderator) {
        ...
        if ($modgroup_pending_count) {
            for ($i = 0; $i < $modgroup_pending_count; $i++) {
                $username = $modgroup_pending_list[$i]['uname'];

                $user_id = $modgroup_pending_list[$i]['uid'];

                generate_user_info(
                    $modgroup_pending_list[$i],
                    $board_config['default_dateformat'],
                    $is_moderator,
                    $from,
                    $posts,
...
ORDER BY g.group_name";
    ...
    while (false !== ($row = $db->sql_fetchrow($result))) {
        if (GROUP_HIDDEN != $row['group_type'] || ADMIN == $userdata['user_level']) {
